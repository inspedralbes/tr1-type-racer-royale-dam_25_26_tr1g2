...
// NUEVO: Semáforo para evitar cargas múltiples
...
async function startCamera (deviceId = '') {
  // FIX: Evitar ejecuciones simultáneas
...
      if (v) {
        v.srcObject = stream
        // FIX: Catch específico para AbortError
        try { 
            await v.play() 
        } catch (e) { 
            if (e.name === 'AbortError') {
                return; // Ignorar interrupción benigna
            }
            console.error('No s’ha pogut iniciar el vídeo:', e) 
        }
      }
...
      await listVideoInputs()
      ...
      isStreamLoading = false; // FIX: Liberar semáforo
  }
}
...
   WATCHERS
------------------------------*/
// Cuando cambiamos de 'camera' a 'file' o viceversa
watch(sourceMode, async (mode) => {
  if (mode === 'camera') {
    // volvemos a la cámara
    await startCamera(selectedId.value || '')
  } else {
    // no hacer nada hasta que el usuario elija un fichero
    stopCurrentSource()
  }
})

// Cuando cambia el deviceId y estamos en modo cámara
watch(selectedId, (id) => {
  if (id && sourceMode.value === 'camera') {
    startCamera(id)
  }
})
</script>
...
        <label><input type="radio" value="camera" name="source" :checked="sourceMode === 'camera'" @change="sourceMode = 'camera'"> Càmera</label>
        <label><input type="radio" value="file" name="source" :checked="sourceMode === 'file'" @change="sourceMode = 'file'"> Vídeo local</label>
      </div>

      <select v-if="sourceMode === 'camera'" v-model="selectedId" class="select">
        <option v-for="d in devices" :key="d.deviceId" :value="d.deviceId">
          {{ d.label }}
        </option>
      </select>
      <input v-if="sourceMode === 'file'" type="file" accept="video/*" @change="e => e.target.files?.[0] && startFileVideo(e.target.files[0])"/>
    </div>
  </div>
</template>
...
/* Contenedor para todos los controles debajo del vídeo */
.controls {
  width: min(100%, 720px);
  display: flex;
  flex-direction: column;
...
/* Selección de fuente (cámara/vídeo) */
.source-select {
  display: flex;
  gap: 16px;
...
/* Estilo común para el select de cámara y el input de fichero */
.select, .controls input[type="file"] {
  width: 100%;
  padding: 6px 8px;
}
</style>